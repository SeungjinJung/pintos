...
void wakeupwaiting(int status){
	...
	struct list_elem *e;
	struct thread *ex=NULL;
	bool end=false;
	if(!list_empty(elist)){
		for(e = list_begin(elist); e!=list_end(elist); e=list_next(e)){
			ex = list_entry(e, struct thread, e_elem);
			if(ex->waitfor == cur->tid){
				end = true;
				break;
			}
		}
		if(end){
			list_remove(&cur->e_elem);
		}
		else{
...
//		printf("%d's exit stats is %d\n", ex->tid, status);
		ex->waitfor = -1;
		ex->child_exit_status = status;
//		printf("%d wake up\n", ex->tid);
		sema_up(&ex->wait);
	}
}
...
static void 
pushespstr(void **esp, void *value, unsigned int length){
	memcpy((char *)(*esp-length), (char *)value, length+1);
	*esp = *esp - length;
}

static void
pushespaddr(void **esp, unsigned int value){
	unsigned int *iesp = (*esp-4);
	*iesp = value;
	*esp = *esp - 0x4;
}
...
start_process (void *f_name)
{
	...
	if(success){
		...
		while(token){
			...
			argvlen += strlen(token)+1;
			argc++;
			token = strtok_r(NULL, " ", &save_ptr);
		}
		pushespstr(&if_.esp, (void *)argv, argvlen);
		//save the argv string's addr
		tempaddr = if_.esp;

		//word-align
		if_.esp -= (unsigned)if_.esp % 4;
		//push 0
		pushespaddr(&if_.esp, 0);

		//substract esp for putting argv's addr
		if_.esp -= 4*argc;
		//save for recovering esp
		save_ptr = if_.esp;
		for(i=0; i<argc; i++){
			*((int *)if_.esp) = (unsigned)tempaddr;
			//			printf("addr %08x ---> %s\n", tempaddr, tempaddr);
			tempaddr += strlen(tempaddr)+1;
			if_.esp += 0x4;
		}
		if_.esp = save_ptr;
		...
		//push wrong ret
		pushespaddr(&if_.esp, 0);
		//hex_dump(if_.esp, (void *)(if_.esp), argvlen+40, 1);
	}
...
//	kpage = palloc_get_page (PAL_USER | PAL_ZERO);
	...
	if (kpage != NULL) 
	{
		...
		if(success){
			//page table에 이 페이지의 정보 추가
			struct pte *pte = make_page_entry(((uint8_t *) PHYS_BASE) - PGSIZE, kpage);
			//stack은 Writable한 공간임
			pte->writable = true;
			pte->loc = MEM;
			insert_page(thread_current()->page_table, pte);
		}
...
install_page (void *upage, void *kpage, bool writable)
{
	...
	//page를 install할때 frame table에 추가
	struct fte *fte = make_frame_entry(upage, kpage, thread_current());
	insert_frame(fte);
